...
export async function setWorkerSyncRef(
  repoRoot: string,
  name: string,
  syncRef: string,
): Promise<void> {
...
export async function advanceWorker(
  ...
  const hostHead = await resolveSyncTarget(repoRoot, worker, state.sourceRef)
...
/**
 * Resolve the commit a worker should advance onto: the tip of its recorded
 * `syncRef` (falling back to the workspace `sourceRef` for workers created
...
async function resolveSyncTarget(
  ...
  const ref = worker.syncRef ?? fallback
  try {
    return await git.revParse(repoRoot, ref)
  } catch {
    throw new QuimbyError(
      `Worker "${worker.name}" syncs against "${ref}", which doesn't resolve in the host repo. ` +
        `Retarget it with "quimby set ${worker.name} --sync <ref>".`,
    )
  }
}
