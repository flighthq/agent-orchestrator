...
Because a detached session is inherently a tmux session, `start` enrolls a local agent into tmux (the `tmux` field) if it wasn't already — so `run`, `nudge`, and `list`'s state all recognize the very session `start` created. SSH agents always use tmux, so nothing changes for them.
