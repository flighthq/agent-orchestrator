...
SSH agents always run in a named tmux session for persistence. Local agents can **opt in** via the `tmux` field on `AgentState` (set through the walkthrough / `config`), or are enrolled automatically by `quimby start` (a headless launch is inherently a tmux session). When set, `quimby run` wraps the local agent in `tmux new-session -A -s <tmuxSessionName>` (attach) and `quimby start` uses `new-session -A -d` (detached, headless) so the session is reattachable. The session name derives from stable IDs (`qb-<projectId[:8]>-<agentId[:8]>`), so it survives renames. `run` and `start` share one launch-prep helper (`apps/cli/src/launch.ts`) — SSH sync + lazy init, runtime spec, bundled tmux config — differing only in attach-vs-detached, and `quimby list` reports each agent's live session state (`getAgentSessionState` → running / attached / stopped).
