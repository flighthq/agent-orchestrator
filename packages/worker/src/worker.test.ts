...
import { getWorkerDir } from '@quimbyhq/paths'
...
describe('addWorker', () => {
  ...
  it('records syncRef defaulting to the host branch', async () => {
    ...
    const worker = await addWorker(dir, 'dana')
    expect(worker.syncRef).toBe(branch.trim())
  })

  it('honors an explicit syncRef', async () => {
    const worker = await addWorker(dir, 'erin', { syncRef: 'release' })
    expect(worker.syncRef).toBe('release')
  })
})

describe('advanceWorker', () => {
  it('throws a clear error when the sync ref does not resolve', async () => {
    await addWorker(dir, 'alice')
    await setWorkerSyncRef(dir, 'alice', 'no-such-branch')
    await expect(advanceWorker(dir, 'alice')).rejects.toThrow(/doesn't resolve/)
  })
})
